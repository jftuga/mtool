...
These items were identified during a security/code review of mtool.
They are improvements and usability fixes, not security issues.
...
(currently 91). Any changes should avoid reducing that count.
...
full path `p` as walked by `filepath.Walk`.
This means archives contain entries like `/home/user/proj/file.txt`
...
**Search for:** `header.Name = p` — there are 3 occurrences
...
**Problem:** In `serveDirectory`, the HTML template at renders
...
function, the entries are built with just `.Name` and `.IsDir`.
...
**Problem:** In `cmdBench`, a `latencyHeap` is created and values
...
**Search for:** `topLatencies` — 3 occurrences (init, heap.Init,
...
**Search for:** `mode.*title` or `ToTitle`.
...
for `cmdFetch` and the `-header` flag definition.
...
**Search for:** `strings.HasPrefix(filepath.Clean` — 3 occurrences.
